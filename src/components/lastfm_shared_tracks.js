...
import { Card, CardDeck, CardTitle, CardImg, CardImgOverlay, Row, Col, CardSubtitle } from 'reactstrap';
import LastFMNoData from './lastfm_no_data';


class LastFMSharedTracks extends Component {
    render() {
        const { sharedTopTracks, username_1, username_2, header_title, noDataHeaderType } = this.props;

        if (!sharedTopTracks) {
            return <div>Loading...</div>;
         }

         const sharedTopTracksRender = (
            <Row>
                <Col>
                    <h3>{header_title}</h3>
                    <hr className="my-2" />
                        ...
                        <Card style={{background: 'transparent', border:0, color: '#fff'}} body inverse>
                            <Row>
                                {_.map(sharedTopTracks, track => {
                                    ...
                                    return(
                                        <Col sm="2" className="p-0 text-left" key={track.user_two_artist_playcount+track.artist_image}> 
        ...
        return (
            <div className="sharedArtistsColumns">
                {!(_.isEmpty(sharedTopTracks)) ? sharedTopTracksRender :  <LastFMNoData username_1={username_1} username_2={username_2} noDataHeaderType={noDataHeaderType} />}
            </div>
        );
    }
}
